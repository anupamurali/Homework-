...
def extract_feats(ffs, direc="train", global_feat_dict=None):
    ...
    for idx, datafile in enumerate(os.listdir(direc)):
        # TEMPORARILY ONLY INCLUDE FIRST 100 POINTS FOR TESTING
        if idx >= TOTAL_NUM_DATA:
           continue
...
def do_validation(train_dir, test_dir, ffs):
    # extract features
    print "extracting training features..."
    X_train,global_feat_dict,t_train,train_ids = extract_feats(ffs, train_dir)
    print "done extracting training features"
    ...
    train_end = int(X_train.shape[0] / 2)
    # TODO train here, and learn your classification parameters
    print "learning..."
    learn.TRAINING_FUNCTION(X_train[:train_end,:], global_feat_dict, t_train[:train_end], train_ids[:train_end])
    print "done learning"
    print

    print "validating..."
    X_test = X_train[train_end:,:]
    preds = learn.TESTING_FUNCTION(X_test, global_feat_dict, train_ids[train_end:])
    ...
    for i in xrange(len(preds)):
        # If prediciton matches true classification...
        if preds[i] == t_train[i + train_end]:
            num_correct[util.malware_classes[t_train[i + train_end]]] += 1
        else:
            # Let classif = what it should've been classified as
            classif = util.malware_classes[t_train[i + train_end]]
            if classif not in incorrect:
                incorrect[classif] = []
            # Add this to the list of incorrect samples for this class
            incorrect[classif].append((train_ids[i + train_end], util.malware_classes[int(preds[i])]))
        num_total[util.malware_classes[t_train[i + train_end]]] += 1
    print "% CORRECT"

    total_correct = 0
    total = 0
    for c in util.malware_classes:
        if num_total[c] > 0:
            print "    {0}: {1} [{2}/{3} correct]".format(c, num_correct[c]*1.0/num_total[c], num_correct[c], num_total[c])
        else:
            print "    {0}: No data".format(c)
        total_correct += num_correct[c]
        total += num_total[c]
    print "TOTAL CORRECT: {0} [{1}/{2} correct]".format(total_correct*1.0/total, total_correct, total)
    print

    # Output incorrect samples to file
    with open("incorrect_samples_output.txt","w") as f:
        for c in util.malware_classes:
            f.write("ALL INCORRECT SAMPLES FOR {0}\n".format(c))
            if c in incorrect:
                for id, wrong in incorrect[c]:
                    f.write("    {0}.{1}.xml classified as {2}\n".format(id, c, wrong))
...
if __name__ == "__main__":
    train_dir = "train"
    test_dir = "test"
    cur_time = strftime("%Y-%m-%d_%I%M%p")
    outputfile = "mypredictions_{0}.csv".format(cur_time)  # feel free to change this or take it as an argument

    output_predictions = False

    if len(sys.argv) > 1:
        TOTAL_NUM_DATA = int(sys.argv[1])
        print "using at most",TOTAL_NUM_DATA,"data points..."
        if len(sys.argv) > 2:
            if sys.argv[2] == "predict":
                output_predictions = True
                print "program will output predictions for Kaggle using filename '{0}'".format(outputfile)
    if not output_predictions:
        print "program will perform validation only"

    # TODO put the names of the feature functions you've defined above in this list
    ffs = features.ALL_FEATURES

    if output_predictions:
        do_prediction(train_dir, test_dir, outputfile, ffs)
    else:
        do_validation(train_dir, test_dir, ffs)
